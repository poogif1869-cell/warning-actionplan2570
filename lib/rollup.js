...
/* ---------------------------------------------------------------------
   ยอดรวมพวกนี้เคยเป็น const คำนวณครั้งเดียวตอน import ก็พอ
   เพราะแผนมาจากไฟล์ที่ฝังในโค้ด ไม่มีทางเปลี่ยนระหว่างใช้งาน

   ตั้งแต่มีถังการแก้ไขข้อมูล (plan_edits) แผนเปลี่ยนได้แล้ว — เพิ่มโครงการ
   ลบโครงการ แก้งบที่จัดสรร ทุกอย่างต้องรวมยอดใหม่หมด จึงเปลี่ยนเป็น let
   แล้วให้ applyPlanEdits() ใน lib/plan.js เรียก rebuildRollups() ต่อทันที

   ใช้ let + export ได้เพราะ ES module เป็น live binding — ไฟล์ที่ import
   ชื่อพวกนี้ไปจะเห็นค่าใหม่เองโดยไม่ต้อง import ซ้ำ
   แต่ค่าที่ถูกจับไว้ใน useMemo(..., []) จะยังค้างของเก่า จึงต้องให้
   หน้าทั้งหน้า remount เมื่อ planVersion เปลี่ยน (ดู app/(app)/shell.jsx)
   --------------------------------------------------------------------- */
export let STRATEGIES = buildStrategies();
export let FUND_ROLLUP = buildFundRollup();
export let PROGRAMS = buildPrograms();
export let ORGS = buildOrgs();
export let ORG_UNITS = buildOrgUnits();
export let ORG_OWNERS = buildOrgOwners();   // ต้องมาหลัง ORG_UNITS เพราะใช้ชื่อจากตัวนั้น
export let RISK_ITEMS = buildRiskItems();

export function rebuildRollups() {
  STRATEGIES = buildStrategies();
  FUND_ROLLUP = buildFundRollup();
  PROGRAMS = buildPrograms();
  ORGS = buildOrgs();
  ORG_UNITS = buildOrgUnits();
  ORG_OWNERS = buildOrgOwners();
  RISK_ITEMS = buildRiskItems();
}
...
function buildRiskItems() {
  return ITEMS.filter((x) => x.rScen || x.rFactor);
}
