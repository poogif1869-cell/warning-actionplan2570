...
export function summarize(alerts) {
  ...
  /* นับ "โครงการ" ที่มีวิกฤต แยกจากการนับ "รายการแจ้งเตือน" ที่เป็นวิกฤต
     หนึ่งโครงการติดวิกฤตได้หลายรายการ ถ้าเอา crit ไปวาดเป็นสัดส่วนของ
     121 โครงการ จะได้ตัวเลขที่รวมกันเกิน 121 ซึ่งอ่านไม่ได้ความ */
  const critUid = new Set();
  let crit = 0;
  let warn = 0;
  let budgetAtRisk = 0;
  ...
  alerts.forEach((a) => {
    ...
    if (a.uid) {
      flagged.add(a.uid);
      if (a.sev === "crit") critUid.add(a.uid);
  ...
  return {
    ...
    critProjects: critUid.size,
    warnOnlyProjects: flagged.size - critUid.size,
  };
}
