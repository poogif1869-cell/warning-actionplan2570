...
/* ---------------------------------------------------------------------
   ...
   เบิกจ่าย: ใช้ของโครงการเองถ้ามีการกรอกไว้ ไม่งั้นจึงรวมของลูก
   เพื่อไม่ให้นับซ้ำสองระดับ ซึ่งเป็นกติกาเดียวกับที่ไฟล์ต้นฉบับใช้กับงบประมาณ
   --------------------------------------------------------------------- */
...
export function rolledReport(results, item) {
  ...
  const ownSpentAll = spentThrough(results, item.uid, 11);
  const useOwn = ownSpentAll > 0;

  function spentTo(month) {
    if (useOwn) return spentThrough(results, item.uid, month);
    return kids.reduce((a, k) => a + spentThrough(results, k.uid, month), 0);
  }
  ...
  return {
    ...
    spentTo,
    fromChildren: !useOwn && kids.length > 0,
  };
}
...
/* ---------------------------------------------------------------------
   ...
   --------------------------------------------------------------------- */
function projectAlerts(results, asOf) {
  const out = [];
...
/* ---------------------------------------------------------------------
   ...
   --------------------------------------------------------------------- */
export function buildAlerts(results, asOf) {
  const all = [...kpiAlerts(results, asOf), ...projectAlerts(results, asOf)];
