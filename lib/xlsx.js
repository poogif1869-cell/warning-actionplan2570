...
export function buildXlsx(sheets) {
  ...
  /* ชื่อชีตซ้ำกัน = Excel เปิดไฟล์ไม่ได้เลย ไม่ใช่แค่แสดงเพี้ยน
     ซ้ำได้ง่ายกว่าที่คิดเพราะ safeSheetName ตัดที่ 31 ตัวอักษร
     ชื่อไทยยาว ๆ สองชื่อที่ขึ้นต้นเหมือนกันจึงกลายเป็นชื่อเดียวกันได้ */
  const used = new Set();
  const names = list.map((s, i) => {
    let n = safeSheetName(s.name, "แผ่น" + (i + 1));
    if (used.has(n)) {
      let k = 2;
