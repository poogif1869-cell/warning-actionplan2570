...
- **ไม่ต้องเลือกรุ่นเอง** เว็บไล่ใช้ `gemini-2.5-flash` → `gemini-3.6-flash`
  → `gemini-2.5-flash-lite` แล้วสลับให้อัตโนมัติเวลาคิวฝั่ง Google เต็ม
  ทับได้ด้วย `GEMINI_MODEL` (ตัวหลัก) และ `GEMINI_MODEL_FALLBACK` (ตัวสำรอง คั่นด้วยจุลภาค)
  เปิด `/api/chat?models=1` ตอนล็อกอินอยู่ เพื่อดูว่าคีย์นี้ใช้รุ่นไหนได้บ้าง
- **ตัวหลักไม่ใช่รุ่นใหม่สุดโดยตั้งใจ** รุ่นใหม่สุดคือรุ่นที่คิวเต็มบ่อยที่สุดเพราะคนแห่ไปใช้
  งานของเว็บนี้คืออ่านตัวเลขจาก JSON แล้วสรุปเป็นภาษาไทย ไม่ต้องใช้รุ่นแรงที่สุด
