...
- เปลี่ยนรุ่นโมเดลได้ด้วย `GEMINI_MODEL` (ค่าเริ่มต้น `gemini-3.6-flash`)
  Google ปลดระวางรุ่นเก่าเป็นระยะ เปิด `/api/chat?models=1` (ตอนล็อกอินอยู่)
  เพื่อดูว่าคีย์นี้ใช้รุ่นไหนได้บ้าง แล้วเอาชื่อนั้นไปตั้ง `GEMINI_MODEL`
  แล้ว Redeploy — แก้ได้โดยไม่ต้องแตะโค้ด
...
ไอคอนสร้างจาก `public/logo.png` ด้วย `powershell -File build\make-icons.ps1`
รันใหม่เมื่อเปลี่ยนโลโก้เท่านั้น และอย่าลืมขยับ `VERSION` ใน `public/sw.js` ด้วย
