...
-- =====================================================================
-- เพิ่มเมื่อ 23 ก.ย. — รายงานผลตัวชี้วัดแบบรายข้อ
--
-- ปัญหา: ไฟล์แผนเขียนตัวชี้วัดหลายข้อรวมในช่องเดียว เช่น
--   "1. ได้ผู้สืบทอดตำแหน่ง 8 ตำแหน่ง 2. พัฒนาพนักงานศักยภาพสูง 9 ราย"
-- เดิมมีช่องรายงานผลกับช่องปัญหาอุปสรรคอย่างละช่องเดียวต่อโครงการ
-- คนกรอกต้องยัดผลของทุกข้อลงช่องเดียว แล้วสรุปไม่ได้ว่าข้อไหนติดปัญหา
--
-- คอลัมน์ใหม่เก็บเป็นอาร์เรย์เรียงตามลำดับข้อที่แยกได้จากข้อความตัวชี้วัด
--   [{ "r": "ผลที่รายงานของข้อ 1", "i": "ปัญหาอุปสรรคของข้อ 1" }, { ... }]
--
-- ⚠️ ตัวชี้วัดที่มีข้อเดียว **ยังใช้คอลัมน์เดิม** (output_result/output_issue)
-- ไม่ย้ายมาอาร์เรย์ เพราะข้อมูลที่กรอกไว้แล้วจะต้องย้ายตามทั้งหมด
-- และรายงาน PDF/Excel ที่อ่านคอลัมน์เดิมจะว่างทันที
-- ส่วนที่มีหลายข้อ ข้อแรกถูกคัดลอกลงคอลัมน์เดิมด้วย เพื่อให้สรุปที่อ่าน
-- คอลัมน์เดิม (เช่น "กิจกรรมนี้รายงานแล้วหรือยัง") ยังทำงานเหมือนเดิม
-- =====================================================================

alter table public.project_results
  add column if not exists output_items jsonb not null default '[]'::jsonb;

alter table public.project_results
  add column if not exists outcome_items jsonb not null default '[]'::jsonb;
