      ...
      <section className="block">
        <h2>
          ...
          <DownloadButton
            ...
            sheets={() => [
              /* ไฟล์ PDF มีทั้งไทล์สรุป ตารางยุทธศาสตร์/กลยุทธ์ และตัวชี้วัด
                 Excel จึงต้องมีครบทั้งสามอย่าง ไม่ใช่มีแต่ตัวชี้วัด */
              {
                name: "สรุปตัวชี้วัด",
                widths: [40, 22, 30],
                rows: [
                  ["รายการ", "ค่า", "หมายเหตุ"],
                  ["ตัวชี้วัดทั้งหมด", KPIS.length, "ค่าเป้าหมายปี 2570"],
                  ["บรรลุเฉลี่ย (%)", summary.avg == null ? "ยังไม่รายงาน" : Math.round(summary.avg * 10) / 10, "จาก " + summary.n + " ตัวที่รายงานแล้ว"],
                  ["บรรลุเป้าหมาย", summary.ok, "ตัวชี้วัด"],
                  ["รายงานผลแล้ว", summary.n, "จาก " + KPIS.length + " ตัว"],
                  ["ต่ำกว่าเป้าหมาย", summary.below, "ตัวชี้วัด"],
                  ["ยังไม่รายงานผล", summary.pending, ""],
                ],
              },
              {
                name: "ยุทธศาสตร์และกลยุทธ์",
                widths: [14, 46, 46, 12, 18],
                rows: [
                  ["ยุทธศาสตร์", "ชื่อยุทธศาสตร์", "กลยุทธ์", "จำนวนโครงการ", "งบประมาณ"],
                  ...STRATEGIES.flatMap((s) => [
                    ["ที่ " + s.no, s.name, "(รวมทั้งยุทธศาสตร์)", s.count, s.budget],
                    ...s.tactics.map((t) => [
                      "ที่ " + s.no,
                      s.name,
                      t.name || "(ไม่ระบุกลยุทธ์)",
                      t.count,
                      t.budget,
                    ]),
                  ]),
